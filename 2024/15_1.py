...
for move in movements:
    # print("=" * 40, " CURRENT MOVE ", move, "=" * 40)
    # print('\n'.join([' '.join(map(str, row)) for row in grid]))
    (dr, dc) = dirs[move]
    (rr, rc) = robot
    (nr, nc) = (rr + dr, rc + dc)
    if grid[nr][nc] == '#': 
        # we just ignore walls and go to next movement
        continue
    elif grid[nr][nc] == 'O':
        # we need to see if we can push further, if not then we ignore
        j, k = nr, nc
        wall = False
        while 0 <= j and j < rows and 0 <= k and k < cols:
            if grid[j][k] == '#':
                wall = True
                break
            if grid[j][k] == '.':
        ...
        if not wall:
            ...
            robot = (nr, nc)
        # print('\n'.join([' '.join(map(str, row)) for row in grid]))
        # how we can see if we can push further? if there's any '.' in the path
        pass
    else:
        robot = (nr, nc)
        grid[rr][rc] = '.'
