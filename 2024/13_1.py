...
for claw_machine in input:
    [button_a, button_b, coords] = claw_machine.split('\n')
    button_a = [int(button.split('+')[-1]) for button in button_a.split(':')[-1].split(', ')]
    button_b = [int(button.split('+')[-1]) for button in button_b.split(':')[-1].split(', ')]
    prize = [int(c.split('=')[-1]) for c in coords.split(': ')[-1].split(', ')]
