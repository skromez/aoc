...
input = [list(row) for row in input.strip().split('\n')]
grid = input
...
def dfs(r, c):
    ...
    if grid[r][c].isdigit():
        ...
        lc = c
        rc = c
        while lc - 1 >= 0 and grid[r][lc - 1].isdigit():
            number = grid[r][lc - 1] + number
            seen.add((r, lc - 1))
            lc -= 1
        while rc + 1 < cols and grid[r][rc + 1].isdigit():
            number = number + grid[r][rc + 1]
            seen.add((r, rc + 1))
            rc += 1
    ...
    for dr, dc in directions:
        ...
        if not grid[nr][nc].isdigit(): continue
        result.append(dfs(nr, nc))
    result = [num for num in result if num > 0]
    return result[0] * result[1] if len(result) == 2 else 0
